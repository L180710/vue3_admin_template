<template>
    ...
    <el-table @selection-change="selectChange" border style="margin: 10px 0" :data="userArr">
      ...
      <el-table-column align="center" label="操作" width="300px">
        <template #="{ row, $index }">
          ...
          <el-popconfirm :title="`您确定要删除${row.username}?`" width="250px" @confirm="deleteUser(row.id)">
            <template #reference>
  ...
  <el-drawer v-model="drawer1">
    ...
    <template #footer>
      <div style="flex: auto">
        <el-button @cli="drawer1 = false">取消</el-button>
...
const handleCheckedCitiesChange = (value: string[]) => {
  // 顶部复选框的勾选数据
  // 代表：勾选上项目的个数与全部职位个数吸顶灯。顶部复选框勾选上
...
const deleteSelectUser = async () => {
  ...
  // 批量删除的请求
  let result: User[] = await reqSelectUser(idList);
  if (result.code == 200) {
    ElMessage({ type: 'success', message: '删除成功' });
    getHasUser(userArr.value.length > 1 ? pageNo.value : pageNo.value - 1);
  }
